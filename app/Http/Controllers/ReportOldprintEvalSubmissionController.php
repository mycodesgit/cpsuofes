...
class ReportOldprintEvalSubmissionController extends Controller
{
    public function subprint_searchresultStore()
    {
        ...
        $currsem = QCEsemester::select('id', 'qceschlyear')
            ...
            ->orderBy('id', 'DESC')
            ->get();
        return view('reports.prntsummaryevalresult.oldevaluationprintsearchresult', compact('currsem'));
    }

    public function getoldevalsubratelistRead(Request $request) 
    {
    ...
    public function getoldevalsubrateprintedlistRead(Request $request) 
    {
        $semester = $request->query('semester');
        $schlyear = $request->query('schlyear');
        $campus = $request->query('campus');
    ...
    public function exportPrintEvalPDF(Request $request)
    {
        ...
        $instrctn = QCEinstruction::where('instructcat', 2)->get();
        ...
        // Generate the PDF
        $pdf = PDF::loadView('reports.prntsummaryevalresult.formpdf.qceformpdfrated', compact('ratingscale',
            'instrctn', 'currsem', 'quest', 'facrated', 'progCod', 'studYear', 'studSec', 'schlyear', 'semester', 'campus', 'facRanck', 'esig'
