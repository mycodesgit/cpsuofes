@extends('layout.app')

@section('body')
    <div class="row ">
        <div class="col-md-12">
            <div class="mb-6">
                <h1 class="fs-3 mb-4">Questions</h1>
                <div class="row">
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-header pt-3">
                                <h6 class="card-title">
                                    <i class="ti ti-plus"></i> Add New
                                </h6>
                            </div>
                            <div class="card-body">
                                <form method="POST" id="addQuestion">
                                    @csrf

                                    <div class="form-group mb-3">
                                        <div class="form-row">
                                            <div class="col-md-12">
                                                <label>Belongs to Category: <span class="text-danger">*</span></label>
                                                <select class="form-control" name="catName_id">
                                                    <option disabled selected> --Select-- </option>
                                                    @foreach($qcecat as $dataqcecat)
                                                        <option value="{{ $dataqcecat->id }}">{{ $dataqcecat->catName }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="form-group mb-3">
                                        <div class="form-row">
                                            <div class="col-md-12">
                                                <label>Question Category: <span class="text-danger">*</span></label>
                                                <select class="form-control" name="questcat">
                                                    <option disabled selected> --Select-- </option>
                                                    <option value="1">Student</option>
                                                    <option value="2">Faculty</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="form-group mb-3">
                                        <div class="form-row">
                                            <div class="col-md-12">
                                                <label>Question: <span class="text-danger">*</span></label>
                                                <textarea class="form-control" rows="4" name="questiontext"></textarea>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <div class="form-row">
                                            <div class="col-md-12">
                                                <button type="submit" class="btn btn-outline-success">
                                                    <i class="fas fa-save"></i> Save
                                                </button>
                                            </div>
                                        </div>
                                    </div>   
                                </form>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-header pt-3">
                                <h6 class="card-title">
                                    <i class="fas fa-list"></i> List
                                </h6>
                            </div>
                            <div class="card-body">
                                <table id="questTable" class="table table-hover">
                                    <thead>
                                        <tr>
                                            <th>Category</th>
                                            <th>Question</th>
                                            <th>Type</th>
                                            <th width="10%">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>

                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="modal fade" id="editQuestModal" tabindex="-1" role="dialog" aria-labelledby="editQuestModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title" id="editQuestModalLabel">Edit Category Name</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form id="editQuestForm">
                    <div class="modal-body">
                        <input type="hidden" name="id" id="editQuestId">
                        <div class="col-md-12 mb-3">
                            <label for="editQuestcatName">Belongs to Category: <span class="text-danger">*</span></label>
                            <select class="form-control" id="editQuestcatName" name="catName_id">
                                @foreach($qcecat as $dataqcecat)
                                    <option value="{{ $dataqcecat->id }}">{{ $dataqcecat->catName }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label>Question Category: <span class="text-danger">*</span></label>
                            <select class="form-control" name="questcat" id="editQuestCats">
                                <option disabled selected> --Select-- </option>
                                <option value="1">Student</option>
                                <option value="2">Faculty</option>
                            </select>
                        </div>
                        <div class="col-md-12">
                            <label for="editQuestName">Question <span class="text-danger">*</span></label>
                            <textarea class="form-control" rows="4" id="editQuestName" name="questiontext"></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-info" data-bs-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-outline-success">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <script>
        var questionReadRoute = "{{ route('question.show') }}";
        var questionCreateRoute = "{{ route('question.create') }}";
        var questionUpdateRoute = "{{ route('question.update', ['id' => ':id']) }}";
        var questionDeleteRoute = "{{ route('question.delete', ['id' => ':id']) }}";
    </script>
@endsection
